 ...
 *
 * The `@alias` decorator factory can be used to create multiple aliases at once, and
 ...
 *
 * The `@alias.for` decorator factory can be used to create single aliases, and
 * should be applied directly to the actual alias, and passed the name of the original
 * class member that it should point to. It can only handle one alias at a time.
 *
...
>(key: Name): AliasDecorator<This, Value, Name>;
...
>(source: Name): AliasDecorator<This, Value, Name> {
  // deno-lint-ignore no-explicit-any
  return (_target, context): any => {
    ...
    if (context.kind === "accessor" && alias.strategy === "accessor") {
      return {
        get(this: This) {
